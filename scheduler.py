...
async def check_and_give_daily_bonuses():
    """Задача: проверить активные подписки и начислить бонусы, если прошли сутки."""
    subscriptions = db.get_all_active_subscriptions()
    now = datetime.now()
    for sub in subscriptions:
        user_id = sub["user_id"]
        last_bonus = sub.get("last_daily_bonus_date")
        # Если last_bonus отсутствует или прошло > 24 часа
...
def start_scheduler():
    """Запускает планировщик с интервалом в 1 час."""
